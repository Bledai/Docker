...
@app.route("/")
def content():
    dateNow = subprocess.run('date "+%Y-%m-%d %H:%M:%S"', shell=True,
                             stdout=subprocess.PIPE).stdout.decode('utf-8')
    time2 = parse_date_bash(dateNow)
    time1 = parse_date_bash(os.getenv('DATE_CREATED'))
    page = '<html><body><h1>'
    page += 'TASK 6'
    page += '</h1>'
    ...
    page += f'Container Life time: {time1 - time2}<br />'
    page += '</body></html>'
